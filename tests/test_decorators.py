"""Tests for django_ormlens.decorators module."""
...
from django_ormlens.decorators import _default_output, _should_sample, explain_query
...
class TestShouldSample:
    ...
    @patch("django_ormlens.decorators.random.random", return_value=0.3)
    def test_samples_below_rate(
        self, mock_random: MagicMock, settings: object
    ) -> None:
        settings.ORMLENS = {"SAMPLE_RATE": 0.5}  # type: ignore[attr-defined]
        assert _should_sample() is True

    @patch("django_ormlens.decorators.random.random", return_value=0.8)
...
class TestDefaultOutput:
    ...
    def test_logs_basic_summary(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with caplog.at_level("INFO", logger="django_ormlens.decorators"):
    ...
    def test_logs_n_plus_one_warning(
        ...
        from django_ormlens.analyzer import N1Detection
        ...
        with caplog.at_level("WARNING", logger="django_ormlens.decorators"):
            _default_output(result, "my_view")
...
class TestExplainQuery:
    ...
    def test_decorator_bare_usage(self, settings: object) -> None:
        ...
        settings.ORMLENS = {"ENABLED": True, "SAMPLE_RATE": 1.0}  # type: ignore[attr-defined]
    ...
    def test_works_even_when_disabled(self, settings: object) -> None:
        ...
        settings.ORMLENS = {"ENABLED": False, "SAMPLE_RATE": 1.0}  # type: ignore[attr-defined]
    ...
    @pytest.mark.django_db
    @patch("django_ormlens.decorators._should_sample", return_value=False)
    def test_skips_when_sampling_misses(
        ...
        settings.ORMLENS = {"ENABLED": True, "SAMPLE_RATE": 0.5}  # type: ignore[attr-defined]
        captured: list[AnalysisResult] = []
    ...
    def test_output_fn_exception_suppressed(
        ...
        rf = RequestFactory()
        with caplog.at_level("ERROR", logger="django_ormlens.decorators"):
            result = my_view(rf.get("/test/"))
